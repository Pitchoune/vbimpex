...
class vb2_005 extends vb2_000
{
		...
		if ($this->check_order($sessionobject, $this->_dependent))
		{
			if ($this->_restart)
			{
				if ($this->restart($sessionobject, $displayobject, $Db_target, $Db_source, 'clear_imported_forums'))
				{
					...
					$displayobject->update_html($displayobject->make_description($displayobject->phrases['forums_cleared']));
	...
	public function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		$displayobject->update_html($displayobject->print_per_page_pass(count($forum_array), $displayobject->phrases['forums_lower'], $forum_start_at));
		...
		if ($forum_array)
		{
			foreach ($forum_array AS $forum_id => $forum_details)
			{
				...
				if ($forum_details['active'])
				{
					$options += 1;
				}

				if ($forum_details['allowposting'])
				{
					$options += 2;
				}

				if ($forum_details['cancontainthreads'])
				{
					$options += 4;
				}
				...
				if ($forum_details['moderateattach'])
				{
					$options += 32;
				}

				if ($forum_details['allowbbcode'])
				{
					$options += 64;
				}

				if ($forum_details['allowimages'])
				{
					$options += 128;
				}

				if ($forum_details['allowhtml'])
				{
					$options += 256;
				}

				if ($forum_details['allowsmilies'])
				{
					$options += 512;
				}

				if ($forum_details['allowicons'])
				{
					$options += 1024;
				}

				if ($forum_details['allowratings'])
				{
					$options += 2048;
				}

				if ($forum_details['countposts'])
				{
					$options += 4096;
				}

				if ($forum_details['styleoverride'])
				{
					$options += 32768;
				}
				...
				if ($try->is_valid())
				{
					...
					{
						$displayobject->update_html($displayobject->make_description('<span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['forum'] . ' -> ' . $try->get_value('mandatory', 'title')));
					...
					else
					{
						...
						$displayobject->update_html($displayobject->make_description($displayobject->phrases['failed'] . ' :: ' . $displayobject->phrases['forum_not_imported']));
					}
				}
				else
				{
		...
		{
			$displayobject->update_html($displayobject->make_description($displayobject->phrases['no_forum_to_import']));
		}
