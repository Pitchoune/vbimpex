		...
		if ($smilie_array)
		{
			foreach ($smilie_array AS $smilie_id => $smilie_details)
			{
				...
				if ($try->is_valid())
				{
					...
					{
						$displayobject->update_html($displayobject->make_description('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> :: ' . $displayobject->phrases['smilie'] . ' -> ' .  $try->get_value('mandatory','smilietext')));
