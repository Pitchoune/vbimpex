		...
		if ($moderator_array)
		{
			foreach ($moderator_array AS $moderator_id => $moderator_details)
			{
				...
				if ($try->is_valid())
				{
					...
					{
						$displayobject->update_html($displayobject->make_description('<span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['moderator'] . ' -> ' . $idcache->get_id('username', $moderator_details['userid'])));
