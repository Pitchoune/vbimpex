...
class vb4_003 extends vb4_000
{
	...
	public function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if ($ranks_array)
		{
			foreach ($ranks_array AS $rank_id => $rank)
			{
				...
				if ($try->is_valid())
				{
					...
					{
						$displayobject->update_html($displayobject->make_description('<span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['rank'] . ' -> ' . $try->get_value('nonmandatory', 'rankimg')));
						$sessionobject->add_session_var($class_num . '_objects_done', intval($sessionobject->get_session_var($class_num . '_objects_done')) + 1);
					}
					else
					{
						...
						$displayobject->update_html($displayobject->make_description($displayobject->phrases['failed'] . ' :: ' . $displayobject->phrases['rank_not_imported']));
					}
				}
				else
				{
		...
		{
			$displayobject->update_html($displayobject->make_description($displayobject->phrases['no_rank_to_import']));
		}
