...
class ImpExDatabase extends ImpExDatabaseCore
{
	...
	* @return	boolean
	*/
	public function clear_imported_cms_articles(&$Db_object, &$databasetype, &$tableprefix)
	{
	...
	* @return	boolean
	*/
	public function clear_imported_cms_sections(&$Db_object, &$databasetype, &$tableprefix)
	{
		...
		while ($nodeinfo = $Db_object->fetch_array($node))
		{
			...
			$Db_object->query("
				DELETE FROM " . $tableprefix . "cms_navigation
	...
	* @return	boolean
	*/
	public function clear_imported_cms_section_orders(&$Db_object, &$databasetype, &$tableprefix)
	{
		$Db_object->query("
			DELETE FROM " . $tableprefix . "cms_sectionorder
			WHERE importid <> 0
		");
		return true;
	}
	...
	* @return	boolean
	*/
	public function clear_imported_cms_widgets(&$Db_object, &$databasetype, &$tableprefix)
	{
		$Db_object->query("
			DELETE FROM " . $tableprefix . "cms_widget
			WHERE importcmswidgetid <> 0
		");

		$Db_object->query("
			DELETE FROM " . $tableprefix . "cms_layoutwidget
			WHERE importid <> 0
		");

		$Db_object->query("
			DELETE FROM " . $tableprefix . "cms_widgetconfig
			WHERE importid <> 0
		");

		$Db_object->query("
			ALTER TABLE " . $tableprefix	 . "cms_widget AUTO_INCREMENT = 0
		");
	...
	* @return	boolean
	*/
	public function clear_imported_cms_categories(&$Db_object, &$databasetype, &$tableprefix)
	{
	...
	* @return	boolean
	*/
	private function delete_cms_category($Db_object, $databasetype, $tableprefix, $catid)
	{
		...
		{
			$Db_object->query("
				DELETE FROM " . $tableprefix . "cms_category
				WHERE catleft
					BETWEEN " . $catinfo['catleft'] . "
					AND " . $catinfo['catright'] . "
			");

			$Db_object->query("
				UPDATE " . $tableprefix . "cms_category SET
					catright = catright - " . $catinfo['catwidth'] . "
				WHERE catright > " . $catinfo['catright'] . "
			");

			$Db_object->query("
				UPDATE " . $tableprefix . "cms_category SET
					catleft = catleft - " . $catinfo['catwidth'] . "
				WHERE catleft > " . $catinfo['catright'] . "
			");

			$Db_object->query("
				ALTER TABLE " . $tableprefix . "cms_category AUTO_INCREMENT = 0
			");
	...
	* @return	boolean
	*/
	private function delete_cms_node($Db_object, $databasetype, $tableprefix, $nodeid)
	{
		...
		{
			$Db_object->query("
				DELETE FROM " . $tableprefix . "cms_node
				WHERE nodeleft
					BETWEEN " . $nodeinfo['nodeleft'] . "
					AND " . $nodeinfo['noderight'] . "
			");

			$Db_object->query("
				UPDATE " . $tableprefix . "cms_node SET
					noderight = noderight - " . $nodeinfo['nodewidth'] . "
				WHERE noderight > " . $nodeinfo['noderight'] . "
			");

			$Db_object->query("
				UPDATE " . $tableprefix . "cms_node SET
					nodeleft = nodeleft - " . $nodeinfo['nodewidth'] . "
				WHERE nodeleft > " . $nodeinfo['noderight'] . "
			");

			$Db_object->query("
				ALTER TABLE " . $tableprefix	 . "cms_node AUTO_INCREMENT = 0
			");
		...
		$Db_object->query("
			DELETE FROM " . $tableprefix . "cms_nodecategory
			WHERE nodeid = " . $nodeid . "
		");

		$Db_object->query("
			DELETE FROM " . $tableprefix . "cms_nodeconfig
			WHERE nodeid = " . $nodeid . "
		");

		$Db_object->query("
			DELETE FROM " . $tableprefix . "cms_nodeinfo
			WHERE nodeid = " . $nodeid . "
		");
	}
	...
	* @return	boolean
	*/
	public function clear_imported_cms_grids(&$Db_object, &$databasetype, &$tableprefix)
	{
		$gridarray = array();
		...
		if ($gridarray)
		{
			$Db_object->query("
				...
				WHERE title IN ('" . implode("', '", $gridarray) . "')
		...
		$Db_object->query("
			DELETE FROM " . $tableprefix . "cms_grid
			WHERE importcmsgridid <> 0
		");

		$Db_object->query("
			ALTER TABLE " . $tableprefix . "cms_grid AUTO_INCREMENT = 0
		");
	...
	* @return	boolean
	*/
	public function clear_imported_cms_layouts(&$Db_object, &$databasetype, &$tableprefix)
	{
		$Db_object->query("
			DELETE FROM " . $tableprefix . "cms_layout
			WHERE importcmslayoutid <> 0
		");

		$Db_object->query("
			ALTER TABLE " . $tableprefix	 . "cms_layout AUTO_INCREMENT = 0
		");
	...
	* @return	boolean
	*/
	public function clear_imported_widgets(&$Db_object, &$databasetype, &$tableprefix)
	{
		$Db_object->query("
			DELETE FROM " . $tableprefix . "cms_widget
			WHERE importcmswidgetid <> 0
	...
	* aaa
	*/
	public function import_cms_category(&$Db_object, &$databasetype, &$tableprefix)
	{
		if (dupe_checking AND !($this->_dupe_checking === false OR $this->_dupe_checking['poll'] === false))
		{
			$there = $Db_object->query_first("
				SELECT importcmscategoryid
				FROM " . $tableprefix . "cms_category
				WHERE importcmscategoryid = " . intval(trim($this->get_value('mandatory', 'importcmscategoryid'))) . "
			");
		...
		$Db_object->query_first("
			UPDATE " . $tableprefix . "cms_category SET
				catright = catright + 2
			WHERE catright >= " . $catleft . "
		");

		$Db_object->query_first("
			UPDATE " . $tableprefix . "cms_category SET
				catleft = catleft + 2
			WHERE catleft >= " . $catleft . "
		");
	...
	* aaa
	*/
	public function import_cms_article(&$Db_object, &$databasetype, &$tableprefix)
	{
		if (dupe_checking AND !($this->_dupe_checking === false OR $this->_dupe_checking['poll'] === false))
		{
	...
	* aaa
	*/
	public function import_cms_section(&$Db_object, &$databasetype, &$tableprefix, &$data, &$nodeinfo, &$nodeconfig, &$nodecategory, &$navigation)
	{
	...
	* aaa
	*/
	public function import_cms_node(&$Db_object, &$databasetype, &$tableprefix, $data = array(), $nodeinfo = null, $nodeconfig = null, $nodecategory = null)
	{
		...
		// TODO: We have to create a node somewhere to put all this stuff under ....... likely not here, but at the beginging and then save it to the session
		// $parentnode is set to 1 for now..

		// Get the new leftnode position
		$parent = $Db_object->query_first("
			SELECT noderight
			FROM " . $tableprefix . "cms_node
			WHERE nodeid = " . intval($this->get_value('nonmandatory', 'parentnode')) . "
		");
		...
		$Db_object->query("
			...
			VALUES
				(" . intval($nodeleft) . ",
				" . intval($noderight) . ",
				" . intval($this->get_value('nonmandatory', 'parentnode')) . ",
	...
	* aaa
	*/
	public function import_cms_widget(&$Db_object, &$databasetype, &$tableprefix, &$data, &$widgetconfig, &$layoutwidget)
	{
		$widgettype = $Db_object->query_first("
			...
			WHERE w.class = '" . addslashes($data['widgettype']['class']) . "'
				AND p.productid = 'vbcms'
		");
	...
	* aaa
	*/
	public function import_cms_nodeinfo(&$Db_object, &$databasetype, &$tableprefix)
	{
	...
	* aaa
	*/
	public function import_cms_section_order(&$Db_object, &$databasetype, &$tableprefix)
	{
		$Db_object->query("
	...
	* aaa
	*/
	public function import_cms_nodeconfig(&$Db_object, &$databasetype, &$tableprefix)
	{
		$Db_object->query("
	...
	* aaa
	*/
	public function import_cms_widgetconfig(&$Db_object, &$databasetype, &$tableprefix)
	{
		$Db_object->query("
	...
	* aaa
	*/
	public function import_cms_nodecategory(&$Db_object, &$databasetype, &$tableprefix)
	{
		$Db_object->query("
	...
	* aaa
	*/
	public function import_cms_layoutwidget(&$Db_object, &$databasetype, &$tableprefix)
	{
		$Db_object->query("
	...
	* aaa
	*/
	public function get_cms_article(&$Db_object, &$databasetype, &$tableprefix, &$start_at, &$per_page)
	{
	...
	* aaa
	*/
	public function get_cms_section(&$Db_object, &$databasetype, &$tableprefix, &$start_at, &$per_page)
	{
		$return_array = array('data' => array());
	...
	* aaa
	*/
	public function get_cms_widget(&$Db_object, &$databasetype, &$tableprefix, &$start_at, &$per_page)
	{
		$return_array = array('data' => array());
	...
	* aaa
	*/
	public function import_cms_grid($Db_object, $databasetype, $tableprefix)
	{
		if (dupe_checking AND !($this->_dupe_checking === false OR $this->_dupe_checking['poll'] === false))
		{
			$there = $Db_object->query_first("
				SELECT importcmsgridid
				FROM " . $tableprefix . "cms_grid
				WHERE importcmsgridid = " . intval(trim($this->get_value('mandatory', 'importcmsgridid'))) . "
			");
		...
		$Db_object->query("
			...
			VALUES
				(" . intval($this->get_value('mandatory', 'importcmsgridid')) . ",
				'" . addslashes($title) . "',
	...
	* aaa
	*/
	public function import_cms_navigation($Db_object, $databasetype, $tableprefix)
	{
		$Db_object->query("
	...
	* aaa
	*/
	public function import_cms_layout($Db_object, $databasetype, $tableprefix)
	{
		if (dupe_checking AND !($this->_dupe_checking === false OR $this->_dupe_checking['poll'] === false))
		{
			$there = $Db_object->query_first("
				SELECT importcmslayoutid
				FROM " . $tableprefix . "cms_layout
				WHERE importcmslayoutid = " . intval(trim($this->get_value('mandatory', 'importcmslayoutid'))) . "
			");
		...
		$Db_object->query("
			...
			VALUES
				(" . intval($this->get_value('mandatory', 'importcmslayoutid')) . ",
	...
	*/
	public function get_cms_category_ids($Db_object, $databasetype, $tableprefix, $pad = 0)
	{
		$categories = $Db_object->query("
			SELECT categoryid, importcmscategoryid
			FROM " . $tableprefix . "cms_category
			WHERE importcmscategoryid > 0
		");
	...
	* @return	array	mixed			Data array[impforumid] = forumid
	*/
	public function get_cms_section_parentnodes($Db_object, $databasetype, $tableprefix, $pad = 0)
	{
		$sections = $Db_object->query("
			SELECT nodeid, importcmsnodeid
			FROM " . $tableprefix . "cms_node
			WHERE importcmsnodeid > 0
				AND issection = 1
		");
	 ...
	 */
	public function cms_move_node($Db_object, $databasetype, $tableprefix, $nodeid, $parentid, $order = false)
	{
		...
		$Db_object->query("
			UPDATE " . $tableprefix . "cms_node SET
				nodeleft = IF (nodeid != " . intval($parent['nodeid']) . ", nodeleft + $src_width, nodeleft),
		...
		if ($source['nodeleft'] > $parent['noderight'])
		{
		 ...
		 $Db_object->query("
			UPDATE " . $tableprefix . "cms_node SET
				nodeleft = nodeleft + " . intval($distance) . ",
		...
		$Db_object->query("
			UPDATE " . $tableprefix . "cms_node SET
				nodeleft = IF(nodeleft >= " . intval($source['nodeleft']) . ", nodeleft - " . intval($src_width) . ", nodeleft),
